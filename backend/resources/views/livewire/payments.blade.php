<div class="space-y-6">
    @if($viewMode === 'index')
        ...
        <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div>
                <p class="text-theme-xs font-semibold uppercase tracking-wider text-brand-500 dark:text-brand-400">{{ __('Collections') }}</p>
                <h1 class="mt-1 text-title-sm font-bold text-gray-800 dark:text-white/90">{{ __('Payments') }}</h1>
                <p class="mt-1 text-theme-sm text-gray-500 dark:text-gray-400">{{ __('Allocate verified collections to open invoices.') }}</p>
            </div>
        ...
        <section class="grid grid-cols-1 gap-4 sm:grid-cols-2 md:gap-6 xl:grid-cols-4">
            <div class="rounded-2xl border border-gray-200 bg-white p-5 dark:border-gray-800 dark:bg-white/[0.03]">
                <p class="text-theme-xs font-semibold uppercase tracking-wider text-gray-500 dark:text-gray-400">{{ __('Total collected') }}</p>
                <p class="mt-2 truncate text-2xl font-bold text-success-600 dark:text-success-500">৳{{ number_format($summary['collected'], 2) }}</p>
                <p class="mt-1 text-theme-xs text-gray-500 dark:text-gray-400">{{ __('All successful payments') }}</p>
            </div>
            <div class="rounded-2xl border border-gray-200 bg-white p-5 dark:border-gray-800 dark:bg-white/[0.03]">
                <p class="text-theme-xs font-semibold uppercase tracking-wider text-gray-500 dark:text-gray-400">{{ __('Today') }}</p>
                <p class="mt-2 truncate text-2xl font-bold text-gray-800 dark:text-white/90">৳{{ number_format($summary['today'], 2) }}</p>
                <p class="mt-1 text-theme-xs text-gray-500 dark:text-gray-400">{{ __('Collections since midnight') }}</p>
            </div>
            <div class="rounded-2xl border border-gray-200 bg-white p-5 dark:border-gray-800 dark:bg-white/[0.03]">
                <p class="text-theme-xs font-semibold uppercase tracking-wider text-gray-500 dark:text-gray-400">{{ __('This month') }}</p>
                <p class="mt-2 truncate text-2xl font-bold text-brand-600 dark:text-brand-400">৳{{ number_format($summary['month'], 2) }}</p>
                <p class="mt-1 text-theme-xs text-gray-500 dark:text-gray-400">{{ __('Collected this month') }}</p>
            </div>
            <div class="rounded-2xl border border-gray-200 bg-white p-5 dark:border-gray-800 dark:bg-white/[0.03]">
                <p class="text-theme-xs font-semibold uppercase tracking-wider text-gray-500 dark:text-gray-400">{{ __('Cash collected') }}</p>
                <p class="mt-2 truncate text-2xl font-bold text-gray-800 dark:text-white/90">৳{{ number_format($summary['cash'], 2) }}</p>
                <p class="mt-1 text-theme-xs text-gray-500 dark:text-gray-400">{{ __('Cash-only collections') }}</p>
            </div>
        </section>

        <!-- Payments table -->
        <x-table heading="{{ __('All payments') }}" :description="$payments->total() === 1 ? __('Showing :count payment', ['count' => number_format($payments->total())]) : __('Showing :count payments', ['count' => number_format($payments->total())])" :paginator="$payments">
            <x-slot:toolbar>
                <div class="flex flex-wrap items-center gap-2">
                    <div class="relative">
                        ...
                        <input type="search" wire:model.live.debounce.300ms="search" placeholder="{{ __('Search invoice, customer, tx id...') }}" class="h-10 w-64 rounded-lg border border-gray-300 bg-transparent py-2 pl-10 pr-3 text-theme-sm text-gray-800 shadow-theme-xs placeholder:text-gray-400 focus:border-brand-300 focus:ring-3 focus:ring-brand-500/10 focus:outline-hidden dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30 dark:focus:border-brand-800">
                    </div>
                    <select wire:model.live="methodFilter" class="h-10 w-40 appearance-none rounded-lg border border-gray-300 bg-transparent px-3.5 py-2 text-theme-sm text-gray-800 shadow-theme-xs focus:border-brand-300 focus:ring-3 focus:ring-brand-500/10 focus:outline-hidden dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:focus:border-brand-800">
                        ...
                        <option value="card">{{ __('Card / Bank') }}</option>
                    </select>
                </div>
            </x-slot:toolbar>
            ...
            <table class="min-w-full">
                <thead class="border-b border-gray-100 bg-gray-50/50 dark:border-gray-800 dark:bg-white/[0.02]">
                    <tr>
                        <th class="px-5 py-3.5 text-left text-theme-xs font-medium uppercase tracking-wide text-gray-500 dark:text-gray-400">{{ __('Payment') }}</th>
                        ...
                        <th class="px-5 py-3.5 text-left text-theme-xs font-medium uppercase tracking-wide text-gray-500 dark:text-gray-400">{{ __('Date') }}</th>
                ...
                <tbody class="divide-y divide-gray-100 dark:divide-gray-800">
                                ...
                                <div class="flex items-center gap-3">
                                    ...
                                    <div class="min-w-0">
                                        ...
                                        @if($payment->transaction_id)
                                            <div class="mt-1 truncate text-theme-xs text-gray-500 dark:text-gray-400">{{ __('Tx :id', ['id' => $payment->transaction_id]) }}</div>
                                        @endif
                                    </div>
                                </div>
                            </td>
                            <td class="px-5 py-4">
                                <span class="text-theme-sm font-medium text-gray-800 dark:text-white/90">{{ $payment->customer->name ?? __('Deleted customer') }}</span>
                                @if($payment->customer)
                                    ...
                                    <div class="flex items-center justify-end gap-2">
                                        <button
                                            ...
                                            @click="$dispatch('confirm-action', {
                                                title: '{{ __('Delete payment') }}',
                                                message: '{{ __('Delete this :method payment of ৳:amount on :invoice? The invoice will be reopened if it was fully paid.', ['method' => $payment->payment_method, 'amount' => number_format((float) $payment->amount, 2), 'invoice' => $payment->invoice?->invoice_number ?? __('an invoice')]) }}',
                                                confirmText: '{{ __('Delete') }}',
                                                wireMethod: 'deletePayment',
                                                wireParams: [{{ $payment->id }}],
                                            })"
                    ...
                    @empty
                        <tr>
                            <td colspan="{{ $canDelete ? 6 : 5 }}" class="px-5 py-12 text-center">
                                <div class="mx-auto max-w-xs">
                                    <p class="text-theme-sm text-gray-500 dark:text-gray-400">{{ $search || $methodFilter ? __('No payments match your filters.') : __('No payments recorded yet.') }}</p>
                                </div>
                            </td>
                        </tr>
                    @endforelse
    ...
    @else
        ...
        <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div>
                <p class="text-theme-xs font-semibold uppercase tracking-wider text-brand-500 dark:text-brand-400">{{ __('Collections') }}</p>
                <h1 class="mt-1 text-title-sm font-bold text-gray-800 dark:text-white/90">{{ __('Record payment') }}</h1>
                <p class="mt-1 text-theme-sm text-gray-500 dark:text-gray-400">{{ __('Allocate a collection to an open invoice.') }}</p>
            </div>
            <div class="flex flex-wrap items-center gap-3">
        ...
        <form wire:submit="save" class="space-y-6">
            ...
            <section class="rounded-2xl border border-gray-200 bg-white p-5 shadow-theme-xs dark:border-gray-800 dark:bg-white/[0.03] sm:p-6">
                <div class="mb-4">
                    <h2 class="text-base font-semibold text-gray-800 dark:text-white/90">{{ __('Invoice & amount') }}</h2>
                    <p class="mt-0.5 text-theme-xs text-gray-500 dark:text-gray-400">{{ __('Choose the open invoice being paid and how much was received.') }}</p>
                </div>

                @if($invoices->isEmpty())
                    <div class="flex items-start gap-3 rounded-xl border border-success-200 bg-success-50 px-4 py-3 dark:border-success-500/20 dark:bg-success-500/10">
                        <svg class="mt-0.5 size-5 shrink-0 stroke-success-500" viewBox="0 0 24 24" fill="none" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M22 11.08V12a10 10 0 1 1-5.93-9.14"/><polyline points="22 4 12 14.01 9 11.01"/></svg>
                        <p class="text-theme-sm text-success-700 dark:text-success-300">{{ __('All invoices are settled. There are no open invoices to record a payment against.') }}</p>
                    </div>
                @else
                    <div>
                        <label for="payment-invoice" class="mb-1.5 block text-theme-sm font-medium text-gray-700 dark:text-gray-400">{{ __('Open invoice') }}<span class="ml-0.5 text-error-500">*</span></label>
                        <x-search-select wireKey="invoice_id" :options="$invoiceOptions" :value="$invoice_id" placeholder="{{ __('Select an open invoice...') }}" :live="true" />
                    ...
                    @if($invoice_id)
                        ...
                        @if($selectedInvoice)
                            <div class="mt-4 grid gap-4 rounded-xl border border-gray-200 bg-gray-50/60 p-4 sm:grid-cols-3 dark:border-gray-800 dark:bg-white/[0.02]">
                                <div>
                                    <p class="text-theme-xs text-gray-500 dark:text-gray-400">{{ __('Invoice') }}</p>
                                    <p class="mt-1 text-theme-sm font-semibold text-gray-800 dark:text-white/90">{{ $selectedInvoice->invoice_number }}</p>
                                </div>
                                <div>
                                    <p class="text-theme-xs text-gray-500 dark:text-gray-400">{{ __('Customer') }}</p>
                                    <p class="mt-1 text-theme-sm font-medium text-gray-800 dark:text-white/90">{{ $selectedInvoice->customer?->name ?? __('Deleted customer') }}</p>
                                </div>
                                <div>
                                    <p class="text-theme-xs text-gray-500 dark:text-gray-400">{{ __('Outstanding') }}</p>
                    ...
                    <div class="mt-4">
                        <label for="payment-amount" class="mb-1.5 block text-theme-sm font-medium text-gray-700 dark:text-gray-400">{{ __('Amount received (BDT)') }}<span class="ml-0.5 text-error-500">*</span></label>
                        ...
                        <p class="mt-1.5 text-theme-xs text-gray-500 dark:text-gray-400">{{ __('Cannot exceed the invoice\'s outstanding balance.') }}</p>
            ...
            @if($invoices->isNotEmpty())
                <section class="rounded-2xl border border-gray-200 bg-white p-5 shadow-theme-xs dark:border-gray-800 dark:bg-white/[0.03] sm:p-6">
                    <div class="mb-4">
                        <h2 class="text-base font-semibold text-gray-800 dark:text-white/90">{{ __('Payment method') }}</h2>
                        <p class="mt-0.5 text-theme-xs text-gray-500 dark:text-gray-400">{{ __('Cash needs no reference. Gateway payments need a transaction ID.') }}</p>
                    </div>
                    <div class="grid gap-4 sm:grid-cols-2">
                        <div>
                            <label for="payment-method" class="mb-1.5 block text-theme-sm font-medium text-gray-700 dark:text-gray-400">{{ __('Method') }}<span class="ml-0.5 text-error-500">*</span></label>
                            <x-search-select wireKey="payment_method" :options="['cash' => __('Cash'), 'bkash' => 'bKash', 'card' => __('Card / Bank')]" :value="$payment_method" placeholder="{{ __('Select method') }}" :searchable="false" :live="true" />
                            @error('payment_method') <p class="mt-1.5 text-theme-xs text-error-600 dark:text-error-400">{{ $message }}</p> @enderror
                        </div>
                        @if($payment_method !== 'cash')
                            <div>
                                <label for="payment-transaction-id" class="mb-1.5 block text-theme-sm font-medium text-gray-700 dark:text-gray-400">{{ __('Transaction ID') }}<span class="ml-0.5 text-error-500">*</span></label>
                                ...
                                <p class="mt-1.5 text-theme-xs text-gray-500 dark:text-gray-400">{{ __('Used for gateway reconciliation and duplicate detection.') }}</p>
            ...
            @if($invoices->isNotEmpty() && $tenant)
                <x-payment-instructions :tenant="$tenant" title="{{ __('What your customer sees') }}" />
            @endif
            ...
            <div class="sticky bottom-4 flex flex-col-reverse gap-3 rounded-2xl border border-gray-200 bg-white/95 px-5 py-4 shadow-theme-lg backdrop-blur sm:flex-row sm:items-center sm:justify-between dark:border-gray-800 dark:bg-gray-900/95">
                ...
                <div class="flex flex-col-reverse gap-3 sm:flex-row">
                    ...
                    <button type="submit" wire:loading.attr="disabled" class="inline-flex items-center justify-center gap-2 rounded-lg bg-brand-500 px-4 py-2.5 text-theme-sm font-medium text-white shadow-theme-xs transition hover:bg-brand-600 disabled:cursor-not-allowed disabled:opacity-50" @disabled($invoices->isEmpty())>
                        <span wire:loading.remove wire:target="save">{{ __('Record payment') }}</span>
