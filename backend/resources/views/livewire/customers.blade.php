<div class="space-y-6">
    @if($viewMode === 'index')
        ...
        <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div>
                <p class="text-theme-xs font-semibold uppercase tracking-wider text-brand-500 dark:text-brand-400">Customers</p>
                <h1 class="mt-1 text-title-sm font-bold text-gray-800 dark:text-white/90">Customer directory</h1>
                <p class="mt-1 text-theme-sm text-gray-500 dark:text-gray-400">Manage subscriber profiles, service status and recurring package assignments.</p>
            </div>
        ...
        <!-- Customers table -->
        <x-table heading="All customers" :description="'Showing '.number_format($customers->total()).' customer'.($customers->total() === 1 ? '' : 's')" :paginator="$customers">
            <x-slot:toolbar>
                <div class="flex flex-wrap items-center gap-2">
                    <div class="relative">
                        ...
                        <input type="search" wire:model.live.debounce.300ms="search" placeholder="Search customers..." class="h-10 w-56 rounded-lg border border-gray-300 bg-transparent py-2 pl-10 pr-3 text-theme-sm text-gray-800 shadow-theme-xs placeholder:text-gray-400 focus:border-brand-300 focus:ring-3 focus:ring-brand-500/10 focus:outline-hidden dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30 dark:focus:border-brand-800">
                    </div>
            ...
            <table class="min-w-full">
                <thead class="border-b border-gray-100 bg-gray-50/50 dark:border-gray-800 dark:bg-white/[0.02]">
                    <tr>
                        ...
                        <th class="px-5 py-3.5 text-left text-theme-xs font-medium uppercase tracking-wide text-gray-500 dark:text-gray-400">Package</th>
                ...
                <tbody class="divide-y divide-gray-100 dark:divide-gray-800">
                    @forelse($customers as $customer)
                        ...
                        <tr class="transition-colors hover:bg-gray-50/70 dark:hover:bg-white/[0.02]">
                            ...
                            <td class="px-5 py-4">
                                <span class="inline-flex rounded-full px-2.5 py-1 text-theme-xs font-medium capitalize {{ $badge }}">{{ $statusLabel }}</span>
                            </td>
                            <td class="px-5 py-4">
                                @if($subscription)
                                    ...
                                    <div class="mt-1 text-theme-xs text-gray-500 dark:text-gray-400">
                                        ৳{{ number_format((float) $subscription->price, 2) }} · {{ ucfirst($subscription->billing_cycle) }}
                                        @if($subscription->next_billing_date)
                                            <span class="text-gray-400 dark:text-gray-500">· Next {{ $subscription->next_billing_date->format('d M Y') }}</span>
                                        @endif
                                    </div>
                                @elseif($customer->package_name)
                                    ...
                                    <div class="mt-1 text-theme-xs text-gray-500 dark:text-gray-400">No active subscription</div>
                                @else
                                    <span class="text-theme-sm text-gray-400 dark:text-gray-500">No package</span>
                                @endif
                            </td>
                            <td class="px-5 py-4">
                                <div class="flex items-center justify-end gap-1.5">
                                    <button type="button" wire:click="edit({{ $customer->id }})" title="Edit customer" class="grid h-8 w-8 place-items-center rounded-lg border border-gray-200 text-gray-500 transition hover:border-brand-300 hover:bg-brand-50 hover:text-brand-600 dark:border-gray-800 dark:text-gray-400 dark:hover:border-brand-500/40 dark:hover:bg-brand-500/10 dark:hover:text-brand-400">
                                    ...
                                    <button
                                        ...
                                        @click="$dispatch('confirm-action', {
                                            title: 'Delete customer',
                                            message: 'Permanently delete {{ $customer->name }}? Any active subscription will be cancelled.',
                                            confirmText: 'Delete',
                                            wireMethod: 'delete',
                        ...
                        <tr>
                            <td colspan="4" class="px-5 py-12 text-center">
                                <div class="mx-auto max-w-xs">
                                    <p class="text-theme-sm text-gray-500 dark:text-gray-400">{{ $search || $statusFilter ? 'No customers match your filters.' : 'No customers found yet.' }}</p>
                                </div>
                            </td>
                        </tr>
                    @endforelse
    ...
    @else
        ...
        <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div>
                <p class="text-theme-xs font-semibold uppercase tracking-wider text-brand-500 dark:text-brand-400">Customers</p>
                <h1 class="mt-1 text-title-sm font-bold text-gray-800 dark:text-white/90">{{ $isEditing ? 'Edit customer' : 'Add customer' }}</h1>
                <p class="mt-1 text-theme-sm text-gray-500 dark:text-gray-400">{{ $isEditing ? 'Update subscriber details and their recurring package.' : 'Register a new subscriber and optionally assign a package.' }}</p>
            </div>
            <div class="flex flex-wrap items-center gap-3">
        ...
        <form wire:submit="save" class="space-y-6">
            ...
            <section class="rounded-2xl border border-gray-200 bg-white p-5 shadow-theme-xs dark:border-gray-800 dark:bg-white/[0.03] sm:p-6">
                <div class="mb-4">
                    <h2 class="text-base font-semibold text-gray-800 dark:text-white/90">Customer information</h2>
                    <p class="mt-0.5 text-theme-xs text-gray-500 dark:text-gray-400">Full name and email are required. These are used to identify the subscriber.</p>
                </div>

                <div>
                    <label for="customer-name" class="mb-1.5 block text-theme-sm font-medium text-gray-700 dark:text-gray-400">Full name<span class="ml-0.5 text-error-500">*</span></label>
                ...
                <div class="mt-4 grid gap-4 sm:grid-cols-2">
                    <div>
                        <label for="customer-email" class="mb-1.5 block text-theme-sm font-medium text-gray-700 dark:text-gray-400">Email address<span class="ml-0.5 text-error-500">*</span></label>
                    ...
                    <div>
                        <label for="customer-phone" class="mb-1.5 block text-theme-sm font-medium text-gray-700 dark:text-gray-400">Mobile number</label>
            ...
            <section class="rounded-2xl border border-gray-200 bg-white p-5 shadow-theme-xs dark:border-gray-800 dark:bg-white/[0.03] sm:p-6">
                <div class="mb-4">
                    <h2 class="text-base font-semibold text-gray-800 dark:text-white/90">Account status</h2>
                    <p class="mt-0.5 text-theme-xs text-gray-500 dark:text-gray-400">Only Active keeps the recurring subscription running.</p>
                </div>
                <div class="grid gap-4 sm:grid-cols-2">
                    <div>
                        <label for="customer-status" class="mb-1.5 block text-theme-sm font-medium text-gray-700 dark:text-gray-400">Current status<span class="ml-0.5 text-error-500">*</span></label>
                        <x-search-select wireKey="status" :options="['active' => 'Active', 'pending' => 'Pending', 'inactive' => 'Inactive', 'suspended' => 'Suspended']" :value="$status" placeholder="Select status" :searchable="false" />
                    ...
                    <div class="flex items-start gap-3 rounded-lg border px-3.5 py-3 {{ $status === 'active' ? 'border-success-100 bg-success-50/70 dark:border-success-500/20 dark:bg-success-500/10' : 'border-gray-200 bg-white dark:border-gray-700 dark:bg-gray-900' }}">
                        ...
                        <p class="text-theme-xs leading-4 text-gray-500 dark:text-gray-400">
                            {{ $status === 'active'
                                ? 'The assigned subscription will be billed on its schedule and kept active.'
                                : 'The assigned subscription will be paused until the customer is set back to Active.' }}
                        </p>
                    </div>
                </div>
            </section>
            ...
            <section class="rounded-2xl border border-gray-200 bg-white p-5 shadow-theme-xs dark:border-gray-800 dark:bg-white/[0.03] sm:p-6">
                <div class="mb-4">
                    <h2 class="text-base font-semibold text-gray-800 dark:text-white/90">Package &amp; billing</h2>
                    <p class="mt-0.5 text-theme-xs text-gray-500 dark:text-gray-400">Choose an active package to enable a recurring billing schedule.</p>
                </div>

                <div>
                    <label for="customer-package" class="mb-1.5 block text-theme-sm font-medium text-gray-700 dark:text-gray-400">Assign package</label>
                    <x-search-select wireKey="package_id" :options="$packageOptions" :value="$package_id" placeholder="Select a package" :live="true" />
                ...
                @if($package_id)
                    <div class="mt-4 rounded-xl border border-gray-200 bg-gray-50/60 p-4 sm:p-5 dark:border-gray-800 dark:bg-white/[0.02]">
                        <div class="mb-4 flex items-center gap-2.5">
                            ...
                            <h3 class="text-theme-sm font-semibold text-gray-800 dark:text-white/90">Billing schedule</h3>
                        </div>
                        <div class="grid gap-4 sm:grid-cols-2">
                            <div>
                                <label for="customer-billing-cycle" class="mb-1.5 block text-theme-sm font-medium text-gray-700 dark:text-gray-400">Billing cycle<span class="ml-0.5 text-error-500">*</span></label>
                                <x-search-select wireKey="billing_cycle" :options="['monthly' => 'Monthly', 'quarterly' => 'Quarterly', 'semiannual' => 'Half-yearly', 'yearly' => 'Yearly']" :value="$billing_cycle" placeholder="Select cycle" :searchable="false" />
                                @error('billing_cycle') <p class="mt-1.5 text-theme-xs text-error-600 dark:text-error-400">{{ $message }}</p> @enderror
                            </div>
                            <div>
                                <label for="customer-next-billing" class="mb-1.5 block text-theme-sm font-medium text-gray-700 dark:text-gray-400">Next billing date<span class="ml-0.5 text-error-500">*</span></label>
                ...
                @else
                    <p class="mt-3 text-theme-xs text-gray-500 dark:text-gray-400">Saving without a package will cancel any existing active subscription.</p>
                @endif
            </section>
            ...
            <div class="sticky bottom-4 flex flex-col-reverse gap-3 rounded-2xl border border-gray-200 bg-white/95 px-5 py-4 shadow-theme-lg backdrop-blur sm:flex-row sm:items-center sm:justify-between dark:border-gray-800 dark:bg-gray-900/95">
                ...
                <div class="flex flex-col-reverse gap-3 sm:flex-row">
                    ...
                    <button type="submit" wire:loading.attr="disabled" class="inline-flex items-center justify-center gap-2 rounded-lg bg-brand-500 px-4 py-2.5 text-theme-sm font-medium text-white shadow-theme-xs transition hover:bg-brand-600 disabled:cursor-not-allowed disabled:opacity-50">
                        <span wire:loading.remove wire:target="save">{{ $isEditing ? 'Save changes' : 'Create customer' }}</span>
