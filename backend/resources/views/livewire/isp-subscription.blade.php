<div class="space-y-6">
    ...
    <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
            ...
            <p class="mt-1 text-theme-sm text-gray-500 dark:text-gray-400">{{ __('Every plan includes all BeeCore features — choose by your ISP size and pay only for what you grow into.') }}</p>
        </div>
    </div>
        ...
        <section class="rounded-2xl border border-gray-200 bg-white p-5 shadow-theme-xs dark:border-gray-800 dark:bg-white/[0.03] sm:p-6">
            <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                <div>
                    ...
                    <h2 class="mt-1 text-title-sm font-bold text-gray-900 dark:text-white">{{ $isSwitchOrder ? __('Change to :name', ['name' => $checkPlan->name]) : __('Subscribe to :name', ['name' => $checkPlan->name]) }}</h2>
                    <p class="mt-1 text-theme-sm text-gray-500 dark:text-gray-400">
                        {{ $isSwitchOrder
                            ? __('Your subscription will change after payment — from :from.', ['from' => $subscription?->plan?->name ?? __('your current plan')])
                            : __('Complete payment below to activate your plan.') }}
                    </p>
                </div>
                    ...
                    <div class="flex h-full flex-col rounded-xl border border-gray-200 bg-gray-50/60 p-5 dark:border-gray-800 dark:bg-white/[0.02]">
                        <div class="flex items-center justify-between gap-3">
                            <div>
                                ...
                                <p class="mt-0.5 text-theme-xs text-gray-500 dark:text-gray-400">{{ $checkCycle === 'yearly' ? __('Yearly billing') : __('Monthly billing') }} · {{ $checkPlan->customer_limit !== null ? __(':count customers', ['count' => number_format($checkPlan->customer_limit)]) : __('Unlimited customers') }}</p>
                            </div>
                        ...
                        <dl class="mt-5 space-y-3 border-t border-gray-200 pt-4 text-theme-sm dark:border-gray-800">
                            ...
                            <div class="flex items-center justify-between gap-3 border-t border-gray-200 pt-3 dark:border-gray-800">
                                <dt class="font-medium text-gray-800 dark:text-white/90">{{ $checkCycle === 'yearly' ? __('Total / year') : __('Total / month') }}</dt>
                            ...
                            @if($checkCycle === 'yearly')
                                <p class="flex items-center gap-1.5 text-theme-xs text-success-600 dark:text-success-400">
                                    ...
                                    {{ __('You save :amount by paying yearly.', ['amount' => '৳'.number_format($cSave, 0)]) }}
                                </p>
                            @endif
                        </dl>
                        ...
                        <div class="mt-4 rounded-lg border border-brand-100 bg-brand-50/60 px-3.5 py-2.5 text-theme-xs text-gray-600 dark:border-brand-500/20 dark:bg-brand-500/10 dark:text-gray-300">
                            @if($isSwitchOrder)
                                {{ __('Online payments are confirmed by the payment gateway; manual orders are activated after the BeeCore Account team verifies your payment.') }}
                            @else
                                {{ __('An invoice for your first :cycle period is raised with this order.', ['cycle' => $checkCycle === 'yearly' ? __('Yearly') : __('Monthly')]) }}
                            @endif
                        </div>
                    </div>
                </div>
                    ...
                    <div class="mt-6 flex flex-col-reverse gap-3 rounded-xl bg-gray-50 px-4 py-3.5 sm:flex-row sm:items-center sm:justify-between dark:bg-white/[0.02]">
                        <p class="text-theme-xs text-gray-500 dark:text-gray-400">
                            @if($chosen && ($chosen['manual'] ?? false))
                                {{ __('After paying, the BeeCore Account team verifies your transfer and activates the plan.') }}
                            @else
                                {{ __('You will be redirected to bKash to pay securely. Your plan activates as soon as bKash confirms the payment.') }}
                            @endif
                        </p>
                        <button type="button" wire:click="confirmCheckout" wire:loading.attr="disabled" class="inline-flex shrink-0 items-center justify-center gap-2 rounded-lg bg-brand-500 px-6 py-2.5 text-theme-sm font-medium text-white shadow-theme-xs transition hover:bg-brand-600 disabled:cursor-not-allowed disabled:opacity-50">
                            <span wire:loading.remove wire:target="confirmCheckout">{{ $chosen && ($chosen['manual'] ?? false) ? __('Submit order — pay :amount', ['amount' => '৳'.number_format($cPrice, 0)]) : __('Pay :amount now', ['amount' => '৳'.number_format($cPrice, 0)]) }}</span>
    ...
    <section class="rounded-2xl border border-gray-200 bg-white p-5 shadow-theme-xs dark:border-gray-800 dark:bg-white/[0.03] sm:p-6">
        <div class="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
            <div class="min-w-0">
                <h2 class="text-base font-semibold text-gray-800 dark:text-white/90">{{ $subscription ? __('Change your plan') : __('Choose your plan') }}</h2>
                <p class="mt-0.5 text-theme-xs text-gray-500 dark:text-gray-400">
                    {{ $billingCycle === 'yearly'
                        ? __('Yearly billing — pay for 12 months at a discounted rate.')
                        : ($subscription
                            ? __('Plans available for your operation type. The new price applies from your next billing period.')
                            : __('Pick a plan below to activate this workspace — an invoice for the first period will be created right away.')) }}
                </p>
            </div>
                ...
                <article class="relative flex flex-col rounded-xl border p-5 transition {{ $isCurrent ? 'border-brand-400 bg-brand-50/50 ring-2 ring-brand-500/15 dark:border-brand-500/60 dark:bg-brand-500/[0.06]' : 'border-gray-200 bg-white hover:border-gray-300 dark:border-gray-800 dark:bg-white/[0.02] dark:hover:border-gray-700' }}">
                    ...
                    @else
                        <button type="button" wire:click="openCheckout({{ $plan->id }})" class="inline-flex w-full items-center justify-center gap-2 rounded-lg bg-brand-500 px-4 py-2.5 text-theme-sm font-medium text-white shadow-theme-xs transition hover:bg-brand-600">
                            ...
                            {{ $subscription ? __('Switch to this plan') : __('Subscribe') }}
                        </button>
                    @endif
                </article>
        @empty
    ...
    @if($subscription)
        @php
            ...
            $statusLabel = __(ucwords(str_replace('_', ' ', $subscription->status)));
        ...
        <section class="grid grid-cols-12 gap-4 md:gap-6">
                ...
                <div class="rounded-2xl border border-gray-200 bg-white p-5 shadow-theme-xs dark:border-gray-800 dark:bg-white/[0.03] sm:p-6">
                    <div class="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                        <div class="flex items-center gap-4">
                            ...
                            <div>
                                <div class="flex flex-wrap items-center gap-2">
                                    <h2 class="text-xl font-bold text-gray-900 dark:text-white">{{ $plan?->name ?? __('Custom plan') }}</h2>
                                    <span class="inline-flex rounded-full px-2.5 py-1 text-theme-xs font-medium capitalize {{ $statusChip }}">{{ $statusLabel }}</span>
                                </div>
                        ...
                        <div class="shrink-0 text-left sm:text-right">
                            ...
                            <p class="text-theme-xs text-gray-500 dark:text-gray-400">{{ $subscription->billing_cycle === 'yearly' ? __('per year') : __('per month') }}</p>
                        </div>
                    </div>
                    ...
                    <div class="mt-6 grid gap-4 rounded-xl border border-gray-200 bg-gray-50/60 p-4 sm:grid-cols-3 dark:border-gray-800 dark:bg-white/[0.02]">
                        ...
                        <div>
                            <p class="text-theme-xs text-gray-500 dark:text-gray-400">{{ __('Renews') }}</p>
                            <p class="mt-1 text-theme-sm font-medium text-gray-800 dark:text-white/90">
                                @if($daysLeft !== null && $daysLeft >= 0)
                                    {{ $subscription->current_period_ends_at->format('d M Y') }} · {{ $daysLeft === 1 ? __('in :count day', ['count' => $daysLeft]) : __('in :count days', ['count' => $daysLeft]) }}
                                @else
                                    {{ __('Expired') }}
                                @endif
                            </p>
                        ...
                        <div>
                            ...
                            <p class="mt-1 text-theme-sm font-medium text-gray-800 dark:text-white/90">{{ $subscription->auto_renew ? __('Enabled') : __('Disabled') }}</p>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="col-span-12 xl:col-span-5">
                <div class="rounded-2xl border border-gray-200 bg-white p-5 shadow-theme-xs dark:border-gray-800 dark:bg-white/[0.03] sm:p-6">
                    <h2 class="text-base font-semibold text-gray-800 dark:text-white/90">{{ __('Plan limits') }}</h2>
                    <p class="mt-0.5 text-theme-xs text-gray-500 dark:text-gray-400">{{ __('Your included usage on this plan.') }}</p>
                    <dl class="mt-4 space-y-3">
                        ...
                        @if($plan?->customer_limit !== null && (float) $plan->overflow_rate > 0)
                            <div class="flex items-center justify-between gap-3 text-theme-sm">
                                <dt class="text-gray-500 dark:text-gray-400">{{ __('Overflow (per extra customer)') }}</dt>
                        ...
                        <div class="flex items-center justify-between gap-3 text-theme-sm">
                            ...
                            <dd class="font-semibold text-gray-800 dark:text-white/90">{{ $plan?->staff_limit !== null ? number_format($plan->staff_limit) : __('Unlimited') }}</dd>
                        </div>
                        <div class="flex items-center justify-between gap-3 text-theme-sm">
                            <dt class="text-gray-500 dark:text-gray-400">{{ __('Resellers') }}</dt>
                            <dd class="font-semibold text-gray-800 dark:text-white/90">{{ $plan?->reseller_limit !== null ? number_format($plan->reseller_limit) : __('Unlimited') }}</dd>
                        </div>
                    </dl>
                    <div class="mt-5 rounded-xl border border-brand-100 bg-brand-50/60 px-4 py-3 dark:border-brand-500/20 dark:bg-brand-500/10">
                        <p class="text-theme-xs leading-5 text-gray-600 dark:text-gray-300">{{ __('Need a custom limit or help with billing? Contact the BeeCore Account team or your BeeCore Sales contact — they can always adjust your plan.') }}</p>
                    </div>
                </div>
            </div>
        </section>
