<div class="space-y-6">
    @if($viewMode === 'index')
        ...
        <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div>
                <p class="text-theme-xs font-semibold uppercase tracking-wider text-brand-500 dark:text-brand-400">Channel operations</p>
                <h1 class="mt-1 text-title-sm font-bold text-gray-800 dark:text-white/90">Resellers</h1>
                <p class="mt-1 text-theme-sm text-gray-500 dark:text-gray-400">Maintain reseller contacts, balances, and access status.</p>
            </div>
        ...
        <section class="grid grid-cols-1 gap-4 sm:grid-cols-2 md:gap-6 xl:grid-cols-4">
            <div class="rounded-2xl border border-gray-200 bg-white p-5 dark:border-gray-800 dark:bg-white/[0.03]">
                ...
                <p class="mt-1 text-theme-xs text-gray-500 dark:text-gray-400">All channel partners</p>
            </div>
            <div class="rounded-2xl border border-gray-200 bg-white p-5 dark:border-gray-800 dark:bg-white/[0.03]">
                <p class="text-theme-xs font-semibold uppercase tracking-wider text-gray-500 dark:text-gray-400">Active</p>
                <p class="mt-2 text-2xl font-bold text-success-600 dark:text-success-500">{{ number_format($summary['active']) }}</p>
                <p class="mt-1 text-theme-xs text-gray-500 dark:text-gray-400">Currently enabled</p>
            </div>
            <div class="rounded-2xl border border-gray-200 bg-white p-5 dark:border-gray-800 dark:bg-white/[0.03]">
                <p class="text-theme-xs font-semibold uppercase tracking-wider text-gray-500 dark:text-gray-400">Suspended</p>
                <p class="mt-2 text-2xl font-bold text-warning-600 dark:text-warning-500">{{ number_format($summary['suspended']) }}</p>
                <p class="mt-1 text-theme-xs text-gray-500 dark:text-gray-400">Access disabled</p>
            </div>
            <div class="rounded-2xl border border-gray-200 bg-white p-5 dark:border-gray-800 dark:bg-white/[0.03]">
                <p class="text-theme-xs font-semibold uppercase tracking-wider text-gray-500 dark:text-gray-400">Combined balance</p>
                <p class="mt-2 truncate text-2xl font-bold text-gray-800 dark:text-white/90">৳{{ number_format($summary['balance'], 2) }}</p>
                <p class="mt-1 text-theme-xs text-gray-500 dark:text-gray-400">Across all accounts</p>
            </div>
        </section>

        <!-- Resellers table -->
        <x-table heading="All resellers" :description="'Showing '.number_format($resellers->total()).' reseller'.($resellers->total() === 1 ? '' : 's')" :paginator="$resellers">
            <x-slot:toolbar>
                <div class="flex flex-wrap items-center gap-2">
                    <div class="relative">
                        ...
                        <input type="search" wire:model.live.debounce.300ms="search" placeholder="Search resellers..." class="h-10 w-56 rounded-lg border border-gray-300 bg-transparent py-2 pl-10 pr-3 text-theme-sm text-gray-800 shadow-theme-xs placeholder:text-gray-400 focus:border-brand-300 focus:ring-3 focus:ring-brand-500/10 focus:outline-hidden dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30 dark:focus:border-brand-800">
                    </div>
                </div>
            </x-slot:toolbar>
            ...
            <table class="min-w-full">
                <thead class="border-b border-gray-100 bg-gray-50/50 dark:border-gray-800 dark:bg-white/[0.02]">
                    <tr>
                        <th class="px-5 py-3.5 text-left text-theme-xs font-medium uppercase tracking-wide text-gray-500 dark:text-gray-400">Reseller</th>
                        <th class="px-5 py-3.5 text-left text-theme-xs font-medium uppercase tracking-wide text-gray-500 dark:text-gray-400">Contact</th>
                        <th class="px-5 py-3.5 text-right text-theme-xs font-medium uppercase tracking-wide text-gray-500 dark:text-gray-400">Balance</th>
                        ...
                        <tr class="transition-colors hover:bg-gray-50/70 dark:hover:bg-white/[0.02]">
                            <td class="px-5 py-4">
                                <div class="flex items-center gap-3">
                                    ...
                                    <div class="min-w-0">
                                        ...
                                        <div class="mt-0.5 text-theme-xs text-gray-500 dark:text-gray-400">Added {{ $reseller->created_at->format('d M Y') }}</div>
                                    </div>
                                </div>
                            </td>
                            <td class="px-5 py-4 text-theme-sm text-gray-600 dark:text-gray-400">
                                <div>{{ $reseller->email ?: 'No email' }}</div>
                                <div class="mt-0.5 text-theme-xs text-gray-500 dark:text-gray-400">{{ $reseller->phone ?: 'No phone' }}</div>
                            </td>
                            ...
                            <td class="px-5 py-4">
                                <button
                                    type="button"
                                    role="switch"
                                    aria-label="{{ $reseller->status === 'active' ? 'Active - click to suspend' : 'Suspended - click to activate' }}"
                                    aria-checked="{{ $reseller->status === 'active' ? 'true' : 'false' }}"
                                    wire:click="toggleStatus({{ $reseller->id }})"
                                    title="{{ $reseller->status === 'active' ? 'Active - click to suspend' : 'Suspended - click to activate' }}"
                                ...
                                <div class="flex items-center justify-end gap-1.5">
                                    ...
                                    <button
                                        ...
                                        @click="$dispatch('confirm-action', {
                                            title: 'Delete reseller',
                                            message: 'Delete {{ $reseller->name }}? Their account and balance record will be removed.',
                                            confirmText: 'Delete',
                                            wireMethod: 'delete',
                        ...
                        <tr>
                            <td colspan="5" class="px-5 py-12 text-center">
                                <div class="mx-auto max-w-xs">
                                    <p class="text-theme-sm text-gray-500 dark:text-gray-400">{{ $search ? 'No resellers match your search.' : 'No resellers created yet.' }}</p>
                                </div>
                            </td>
                        </tr>
                    @endforelse
    ...
    @else
        ...
        <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div>
                <p class="text-theme-xs font-semibold uppercase tracking-wider text-brand-500 dark:text-brand-400">Channel operations</p>
                <h1 class="mt-1 text-title-sm font-bold text-gray-800 dark:text-white/90">{{ $isEditing ? 'Edit reseller' : 'Add reseller' }}</h1>
                <p class="mt-1 text-theme-sm text-gray-500 dark:text-gray-400">{{ $isEditing ? 'Update contact details and access status.' : 'Register a new channel partner.' }}</p>
            </div>
            <div class="flex flex-wrap items-center gap-3">
        ...
        <form wire:submit="save" class="space-y-6">
            ...
            <section class="rounded-2xl border border-gray-200 bg-white p-5 shadow-theme-xs dark:border-gray-800 dark:bg-white/[0.03] sm:p-6">
                <div class="mb-4">
                    <h2 class="text-base font-semibold text-gray-800 dark:text-white/90">Contact details</h2>
                    <p class="mt-0.5 text-theme-xs text-gray-500 dark:text-gray-400">Name is required. Email and phone are optional but recommended.</p>
                </div>
                <div>
                    <label for="reseller-name" class="mb-1.5 block text-theme-sm font-medium text-gray-700 dark:text-gray-400">Full name<span class="ml-0.5 text-error-500">*</span></label>
                    <input id="reseller-name" type="text" wire:model="name" class="h-11 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-theme-sm text-gray-800 shadow-theme-xs placeholder:text-gray-400 focus:border-brand-300 focus:ring-3 focus:ring-brand-500/10 focus:outline-hidden dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30" placeholder="e.g. Aziz Traders">
                    @error('name') <p class="mt-1.5 text-theme-xs text-error-600 dark:text-error-400">{{ $message }}</p> @enderror
                </div>
                <div class="mt-4 grid gap-4 sm:grid-cols-2">
                    <div>
                        <label for="reseller-email" class="mb-1.5 block text-theme-sm font-medium text-gray-700 dark:text-gray-400">Email address</label>
                    ...
                    <div>
                        <label for="reseller-phone" class="mb-1.5 block text-theme-sm font-medium text-gray-700 dark:text-gray-400">Phone number</label>
            ...
            <section class="rounded-2xl border border-gray-200 bg-white p-5 shadow-theme-xs dark:border-gray-800 dark:bg-white/[0.03] sm:p-6">
                <div class="mb-4">
                    <h2 class="text-base font-semibold text-gray-800 dark:text-white/90">Access status</h2>
                    <p class="mt-0.5 text-theme-xs text-gray-500 dark:text-gray-400">Suspended resellers cannot place orders or use the portal.</p>
                </div>
                <div class="grid gap-4 sm:grid-cols-2">
                    <div>
                        <label for="reseller-status" class="mb-1.5 block text-theme-sm font-medium text-gray-700 dark:text-gray-400">Status<span class="ml-0.5 text-error-500">*</span></label>
                    ...
                    <div class="flex items-start gap-3 rounded-lg border px-3.5 py-3 {{ $status === 'active' ? 'border-success-100 bg-success-50/70 dark:border-success-500/20 dark:bg-success-500/10' : 'border-gray-200 bg-white dark:border-gray-700 dark:bg-gray-900' }}">
                        ...
                        <p class="text-theme-xs leading-4 text-gray-500 dark:text-gray-400">
                            {{ $status === 'active'
                                ? 'The reseller is enabled and can transact normally.'
                                : 'The reseller is suspended and cannot transact until reactivated.' }}
                        </p>
                    </div>
                </div>
            </section>
            ...
            <div class="sticky bottom-4 flex flex-col-reverse gap-3 rounded-2xl border border-gray-200 bg-white/95 px-5 py-4 shadow-theme-lg backdrop-blur sm:flex-row sm:items-center sm:justify-between dark:border-gray-800 dark:bg-gray-900/95">
                ...
                <div class="flex flex-col-reverse gap-3 sm:flex-row">
                    ...
                    <button type="submit" wire:loading.attr="disabled" class="inline-flex items-center justify-center gap-2 rounded-lg bg-brand-500 px-4 py-2.5 text-theme-sm font-medium text-white shadow-theme-xs transition hover:bg-brand-600 disabled:cursor-not-allowed disabled:opacity-50">
                        <span wire:loading.remove wire:target="save">{{ $isEditing ? 'Save changes' : 'Add reseller' }}</span>
