<div class="space-y-6">
    ...
    <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
            ...
            <h1 class="mt-1 text-title-sm font-bold text-gray-800 dark:text-white/90">ISP settings</h1>
            <p class="mt-1 text-theme-sm text-gray-500 dark:text-gray-400">Company profile, invoice &amp; billing rules for this workspace.</p>
        </div>
        <div class="inline-flex shrink-0 items-center gap-2 rounded-lg border border-gray-200 bg-white px-3 py-2 text-theme-xs font-medium text-gray-500 shadow-theme-xs dark:border-gray-800 dark:bg-white/[0.03] dark:text-gray-400">
            ...
            {{ $workspace->operationModeLabel() }} operations
        </div>
    </div>
    ...
    <form wire:submit="save" class="space-y-6">
        ...
        <section class="rounded-2xl border border-gray-200 bg-white p-5 shadow-theme-xs dark:border-gray-800 dark:bg-white/[0.03] sm:p-6">
            <div class="mb-4">
                <h2 class="text-base font-semibold text-gray-800 dark:text-white/90">Company profile</h2>
                <p class="mt-0.5 text-theme-xs text-gray-500 dark:text-gray-400">Shown to your customers on invoices and the customer app.</p>
            </div>
            <div class="grid gap-4 sm:grid-cols-2">
                <div>
                    <label for="settings-name" class="mb-1.5 block text-theme-sm font-medium text-gray-700 dark:text-gray-400">Company / business name<span class="ml-0.5 text-error-500">*</span></label>
                ...
                <div>
                    <label for="settings-phone" class="mb-1.5 block text-theme-sm font-medium text-gray-700 dark:text-gray-400">Contact phone</label>
                ...
                <div class="sm:col-span-2">
                    <label for="settings-address" class="mb-1.5 block text-theme-sm font-medium text-gray-700 dark:text-gray-400">Business address</label>
                ...
                <div>
                    <label for="settings-currency" class="mb-1.5 block text-theme-sm font-medium text-gray-700 dark:text-gray-400">Currency<span class="ml-0.5 text-error-500">*</span></label>
                ...
                <div>
                    <label for="settings-timezone" class="mb-1.5 block text-theme-sm font-medium text-gray-700 dark:text-gray-400">Timezone<span class="ml-0.5 text-error-500">*</span></label>
                ...
                <div>
                    <label for="settings-language" class="mb-1.5 block text-theme-sm font-medium text-gray-700 dark:text-gray-400">Default language<span class="ml-0.5 text-error-500">*</span></label>
        ...
        <section class="rounded-2xl border border-gray-200 bg-white p-5 shadow-theme-xs dark:border-gray-800 dark:bg-white/[0.03] sm:p-6">
            <div class="mb-4 flex items-start gap-3">
                ...
                <div>
                    <h2 class="text-base font-semibold text-gray-800 dark:text-white/90">Workspace owner</h2>
                    <p class="mt-0.5 text-theme-xs text-gray-500 dark:text-gray-400">The account that owns this workspace. Managed from My Profile.</p>
                </div>
            </div>
            <div class="grid gap-4 rounded-xl border border-gray-200 bg-gray-50/60 p-4 sm:grid-cols-3 dark:border-gray-800 dark:bg-white/[0.02]">
                ...
                <div>
                    <p class="text-theme-xs text-gray-500 dark:text-gray-400">Phone</p>
        ...
        <section class="rounded-2xl border border-gray-200 bg-white p-5 shadow-theme-xs dark:border-gray-800 dark:bg-white/[0.03] sm:p-6">
            <div class="mb-4">
                <h2 class="text-base font-semibold text-gray-800 dark:text-white/90">Invoice &amp; billing rules</h2>
                <p class="mt-0.5 text-theme-xs text-gray-500 dark:text-gray-400">How recurring invoices are dated, when they go overdue, and when unpaid customers get suspended.</p>
            </div>

            <div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                <div>
                    <label for="settings-grace" class="mb-1.5 block text-theme-sm font-medium text-gray-700 dark:text-gray-400">Invoice due after (days)</label>
                    ...
                    <p class="mt-1.5 text-theme-xs text-gray-500 dark:text-gray-400">Recurring invoices are due this many days after the billing period starts.</p>
                    @error('graceDays') <p class="mt-1.5 text-theme-xs text-error-600 dark:text-error-400">{{ $message }}</p> @enderror
                </div>

                <div>
                    <label for="settings-cutoff" class="mb-1.5 block text-theme-sm font-medium text-gray-700 dark:text-gray-400">Billing cutoff day</label>
                    ...
                    <p class="mt-1.5 text-theme-xs text-gray-500 dark:text-gray-400">Day of the month new subscriptions start billing from (1–28).</p>
                    @error('cutoffDay') <p class="mt-1.5 text-theme-xs text-error-600 dark:text-error-400">{{ $message }}</p> @enderror
                </div>

                <div>
                    <label for="settings-suspend-days" class="mb-1.5 block text-theme-sm font-medium text-gray-700 dark:text-gray-400">Suspend after (days past due)</label>
                    ...
                    <p class="mt-1.5 text-theme-xs text-gray-500 dark:text-gray-400">Unpaid customers are suspended automatically after this many days.</p>
            ...
            <div class="mt-4 flex items-start justify-between gap-4 rounded-xl border border-gray-200 p-4 sm:items-center dark:border-gray-800">
                <div class="flex items-start gap-3">
                    ...
                    <div>
                        <p class="text-theme-sm font-medium text-gray-800 dark:text-white/90">Auto-suspend unpaid customers</p>
                        <p class="mt-0.5 max-w-md text-theme-xs text-gray-500 dark:text-gray-400">When a customer stays unpaid past the due date for the configured period, suspend their service automatically. Works alongside manual suspension.</p>
                    </div>
                </div>
                <button
                    type="button"
            ...
            <div class="mt-4">
                <label for="settings-terms" class="mb-1.5 block text-theme-sm font-medium text-gray-700 dark:text-gray-400">Invoice terms &amp; footer note</label>
                <textarea id="settings-terms" wire:model="invoiceTerms" rows="3" class="w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-theme-sm text-gray-800 shadow-theme-xs placeholder:text-gray-400 focus:border-brand-300 focus:ring-3 focus:ring-brand-500/10 focus:outline-hidden dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30" placeholder="e.g. Payment is due within the stated period. Late payments may result in service suspension."></textarea>
                <p class="mt-1.5 text-theme-xs text-gray-500 dark:text-gray-400">Shown at the bottom of customer invoices (print/PDF).</p>
        ...
        <div class="sticky bottom-4 flex flex-col-reverse gap-3 rounded-2xl border border-gray-200 bg-white/95 px-5 py-4 shadow-theme-lg backdrop-blur sm:flex-row sm:items-center sm:justify-between dark:border-gray-800 dark:bg-gray-900/95">
            <p class="text-theme-xs text-gray-500 dark:text-gray-400">Changes apply to new invoices and workspace defaults immediately.</p>
