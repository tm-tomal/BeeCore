@props(['tenant' => null, 'title' => 'Payment instructions'])

@php
    $config = $tenant?->settings['collection'] ?? [];
    $mode = $config['mode'] ?? 'bee';
    $fee = (int) ($config['bee_fee_percent'] ?? 2);
    ...
    @if($mode === 'bee')
        <div class="mt-2 flex items-start gap-2.5">
            ...
            <p class="text-theme-xs leading-5 text-gray-600 dark:text-gray-400">
                Pay securely through the <span class="font-medium text-gray-800 dark:text-white/90">Bee Payment Gateway</span>. A {{ $fee }}% processing fee applies.
            </p>
        </div>
    @else
        <div class="mt-2 space-y-2">
            ...
            @endphp
            @if(! $hasMethod)
                <p class="text-theme-xs text-gray-500 dark:text-gray-400">Pay directly to this ISP. Contact them for account details.</p>
            @endif
