<div class="space-y-6">
    ...
    <header class="flex flex-col gap-3 xl:flex-row xl:items-end xl:justify-between">
        <div>
            <p class="text-theme-xs font-semibold uppercase tracking-wider text-brand-500 dark:text-brand-400">{{ __('Analytics') }}</p>
    ...
    <section class="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-4 md:gap-6">
        <x-dashboard-metric icon="wallet" label="{{ __('Collections') }}" :count="$metrics['collections']" decimals="2" currency value="{{ number_format($metrics['collections'], 2) }}" sub="{{ __(':count transactions', ['count' => number_format($metrics['transactions'])]) }}" />
        <x-dashboard-metric icon="receipt" label="{{ __('Invoiced') }}" :count="$metrics['invoiced']" decimals="2" currency value="{{ number_format($metrics['invoiced'], 2) }}" :trend="__(':pct% collected', ['pct' => $metrics['collection_rate']])" sub="{{ __('Selected period') }}" />
        <x-dashboard-metric icon="users" label="{{ __('Customers') }}" :count="$metrics['customers']" value="{{ number_format($metrics['customers']) }}" :trend="__(':count active', ['count' => $metrics['active_customers']])" sub="{{ __('Total in workspace') }}" />
    ...
    <section class="rounded-2xl border border-gray-200 bg-white p-4 shadow-theme-xs dark:border-gray-800 dark:bg-white/[0.03] sm:px-6">
        <div class="grid grid-cols-2 gap-4 md:grid-cols-4">
            <div class="flex items-center justify-between gap-3">
                <span class="text-theme-xs font-medium text-gray-500 dark:text-gray-400">{{ __('Collection rate') }}</span>
            ...
            <div class="flex items-center justify-between gap-3">
                <span class="text-theme-xs font-medium text-gray-500 dark:text-gray-400">{{ __('Average payment') }}</span>
                <span class="text-theme-sm font-bold text-gray-800 dark:text-white/90">৳{{ number_format($metrics['avg_payment'], 2) }}</span>
            </div>
            <div class="flex items-center justify-between gap-3">
                <span class="text-theme-xs font-medium text-gray-500 dark:text-gray-400">{{ __('Top method') }}</span>
            ...
            <div class="flex items-center justify-between gap-3">
                <span class="text-theme-xs font-medium text-gray-500 dark:text-gray-400">{{ __('Top method value') }}</span>
    ...
    <section class="grid grid-cols-1 gap-4 xl:grid-cols-2 md:gap-6">
        <div class="rounded-2xl border border-gray-200 bg-white p-5 dark:border-gray-800 dark:bg-white/[0.03] sm:p-6">
            <div class="mb-5 flex items-center justify-between gap-3">
                <div>
                    <h2 class="text-base font-semibold text-gray-800 dark:text-white/90">{{ __('Collections by method') }}</h2>
                    <p class="mt-0.5 text-theme-xs text-gray-500 dark:text-gray-400">{{ __('How money is coming in this period.') }}</p>
                </div>
                        ...
                        <div>
                            <div class="mb-2 flex items-center justify-between gap-4 text-theme-sm">
                                <span class="flex min-w-0 items-center gap-2.5">
                                    ...
                                    <span class="shrink-0 rounded-full bg-gray-100 px-2 py-0.5 text-theme-xs text-gray-500 dark:bg-white/[0.05] dark:text-gray-400">{{ __(':count payments', ['count' => $method['transactions']]) }}</span>
                                </span>
            ...
            @else
                <div class="rounded-xl border border-dashed border-gray-200 py-12 text-center dark:border-gray-800">
                    <p class="text-theme-sm text-gray-500 dark:text-gray-400">{{ __('No payments in this period.') }}</p>
                </div>
            @endif
        </div>
        ...
        <div class="rounded-2xl border border-gray-200 bg-white p-5 dark:border-gray-800 dark:bg-white/[0.03] sm:p-6">
            <div class="mb-5 flex items-center justify-between gap-3">
                <div>
                    <h2 class="text-base font-semibold text-gray-800 dark:text-white/90">{{ __('Invoice status') }}</h2>
                    <p class="mt-0.5 text-theme-xs text-gray-500 dark:text-gray-400">{{ __('Count and value by status this period.') }}</p>
                </div>
                    ...
                    @foreach($invoiceStatuses as $row)
                        ...
                        <div class="flex items-center justify-between gap-4 rounded-xl border border-gray-200 bg-gray-50/40 px-4 py-3.5 dark:border-gray-800 dark:bg-white/[0.02]">
                            <span class="inline-flex items-center gap-2.5">
                                ...
                                <span class="text-theme-xs text-gray-500 dark:text-gray-400">{{ $row['count'] }} {{ $row['count'] === 1 ? __('invoice') : __('invoices') }}</span>
                            </span>
            ...
            @else
                <div class="rounded-xl border border-dashed border-gray-200 py-12 text-center dark:border-gray-800">
                    <p class="text-theme-sm text-gray-500 dark:text-gray-400">{{ __('No invoices in this period.') }}</p>
                </div>
            @endif
        </div>
    </section>
    ...
    <p class="flex items-center justify-center gap-1.5 text-theme-xs text-gray-400 dark:text-gray-500">
        ...
        {{ __('Download PDF opens a clean black & white report for the current date range.') }}
    </p>
</div>
