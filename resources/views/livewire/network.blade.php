<div class="space-y-6">
    ...
    <header class="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        <div>
            <p class="text-theme-xs font-semibold uppercase tracking-wider text-brand-500 dark:text-brand-400">{{ __('Infrastructure') }}</p>
            <h1 class="mt-1 text-title-sm font-bold text-gray-800 dark:text-white/90">{{ __('Network devices') }}</h1>
            <p class="mt-1 text-theme-sm text-gray-500 dark:text-gray-400">{{ __('Track routers, access systems, locations, and operating state.') }}</p>
        </div>
        ...
        <div class="w-full overflow-x-auto">
            <table class="w-full">
                <thead class="border-b border-gray-100 bg-gray-50/60 dark:border-gray-800">
                    <tr>
                        <th class="px-5 py-3.5 text-left text-theme-xs font-medium uppercase tracking-wider text-gray-500 dark:text-gray-400">{{ __('Device') }}</th>
                        <th class="px-5 py-3.5 text-left text-theme-xs font-medium uppercase tracking-wider text-gray-500 dark:text-gray-400">{{ __('IP address') }}</th>
                        <th class="px-5 py-3.5 text-left text-theme-xs font-medium uppercase tracking-wider text-gray-500 dark:text-gray-400">{{ __('Type') }}</th>
                        <th class="px-5 py-3.5 text-left text-theme-xs font-medium uppercase tracking-wider text-gray-500 dark:text-gray-400">{{ __('Location') }}</th>
                        <th class="px-5 py-3.5 text-right text-theme-xs font-medium uppercase tracking-wider text-gray-500 dark:text-gray-400">{{ __('Status') }}</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-gray-100 dark:divide-gray-800">
                    @forelse($devices as $device)
                        <tr class="transition-colors hover:bg-gray-50 dark:hover:bg-white/[0.02]">
                            ...
                            <td class="px-5 py-4 align-middle text-theme-sm text-gray-600 dark:text-gray-400">{{ $device->location ?: __('Not set') }}</td>
                            <td class="px-5 py-4 align-middle text-right">
                                @if($device->status === 'online')
                                    <span class="rounded-full bg-success-50 px-2.5 py-0.5 text-theme-xs font-medium text-success-600 dark:bg-success-500/15 dark:text-success-500">{{ __(ucfirst($device->status)) }}</span>
                                @elseif($device->status === 'maintenance')
                                    <span class="rounded-full bg-warning-50 px-2.5 py-0.5 text-theme-xs font-medium text-warning-600 dark:bg-warning-500/15 dark:text-warning-500">{{ __(ucfirst($device->status)) }}</span>
                                @else
                                    <span class="rounded-full bg-error-50 px-2.5 py-0.5 text-theme-xs font-medium text-error-600 dark:bg-error-500/15 dark:text-error-500">{{ __(ucfirst($device->status)) }}</span>
                                @endif
                            </td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="5" class="px-5 py-12 text-center text-theme-sm text-gray-500 dark:text-gray-400">{{ __('No network devices configured.') }}</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
        ...
        <div class="fixed inset-0 z-[60] flex items-center justify-center p-4" role="dialog" aria-modal="true" aria-labelledby="network-dialog-title">
            ...
            <div class="relative max-h-[92vh] w-full max-w-lg overflow-y-auto rounded-2xl border border-gray-200 bg-white p-5 shadow-2xl sm:p-6 dark:border-gray-800 dark:bg-gray-900">
                <div class="mb-6 flex items-start justify-between gap-3">
                    <div>
                        <h3 id="network-dialog-title" class="text-lg font-semibold text-gray-800 dark:text-white/90">{{ __('Add network device') }}</h3>
                        <p class="mt-1 text-theme-sm text-gray-500 dark:text-gray-400">{{ __('Store inventory details only. Credentials are never collected here.') }}</p>
                    </div>
                ...
                <form wire:submit="save" class="space-y-5">
                    <div>
                        <label for="device-name" class="mb-1.5 block text-theme-sm font-medium text-gray-700 dark:text-gray-400">{{ __('Device name') }}</label>
                    ...
                    <div class="grid gap-5 sm:grid-cols-2">
                        <div>
                            <label for="device-ip" class="mb-1.5 block text-theme-sm font-medium text-gray-700 dark:text-gray-400">{{ __('IP address') }}</label>
                        ...
                        <div>
                            <label for="device-type" class="mb-1.5 block text-theme-sm font-medium text-gray-700 dark:text-gray-400">{{ __('Device type') }}</label>
                    ...
                    <div>
                        <label for="device-location" class="mb-1.5 block text-theme-sm font-medium text-gray-700 dark:text-gray-400">{{ __('Location') }}</label>
                    ...
                    <div>
                        <label for="device-status" class="mb-1.5 block text-theme-sm font-medium text-gray-700 dark:text-gray-400">{{ __('Status') }}</label>
                    ...
                    <div class="flex justify-end gap-3 border-t border-gray-100 pt-5 dark:border-gray-800">
                        ...
                        <button type="submit" wire:loading.attr="disabled" class="inline-flex items-center justify-center gap-2 rounded-lg bg-brand-500 px-4 py-2.5 text-theme-sm font-medium text-white shadow-theme-xs transition hover:bg-brand-600 disabled:cursor-not-allowed disabled:opacity-50">
                            <span wire:loading.remove wire:target="save">{{ __('Save device') }}</span>
