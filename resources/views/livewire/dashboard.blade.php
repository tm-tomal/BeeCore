<div class="space-y-6">
    ...
    <section class="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-4 md:gap-6">
        @if($isSaasView)
            <x-dashboard-metric icon="buildings" label="{{ __('Active tenants') }}" :count="$metrics['tenants']" value="{{ number_format($metrics['tenants']) }}" :trend="__(':value this month', ['value' => '+'.$metrics['new_tenants']])" :sub="__(':trial trial · :suspended suspended', ['trial' => $metrics['trial_tenants'], 'suspended' => $metrics['suspended_tenants']])" />
            <x-dashboard-metric icon="wallet" label="{{ __('SaaS MRR') }}" :count="$metrics['saas_mrr']" decimals="2" currency value="{{ number_format($metrics['saas_mrr'], 2) }}" :sub="__('৳:value ARR', ['value' => number_format($metrics['saas_arr'], 2)])" />
            <x-dashboard-metric icon="clock" label="{{ __('Expiring soon') }}" :count="$metrics['subscriptions_expiring']" value="{{ number_format($metrics['subscriptions_expiring']) }}" sub="{{ __('Next 30 days') }}" />
            <x-dashboard-metric icon="check" label="{{ __('SaaS collected') }}" :count="$metrics['saas_collected_this_month']" decimals="2" currency value="{{ number_format($metrics['saas_collected_this_month'], 2) }}" sub="{{ __(':value last month', ['value' => '৳'.number_format($metrics['saas_collected_last_month'], 2)]) }}" :trend="__(':value vs last month', ['value' => ($metrics['saas_collected_delta'] >= 0 ? '+' : '').$metrics['saas_collected_delta'].'%'])" :trend-up="$metrics['saas_collected_delta'] >= 0" />
        @else
            <x-dashboard-metric icon="users" label="{{ __('Customers') }}" :count="$metrics['customers']" value="{{ number_format($metrics['customers']) }}" :trend="__(':value this month', ['value' => '+'.$metrics['new_customers']])" :sub="__(':active active · :suspended suspended', ['active' => $metrics['active_customers'], 'suspended' => $metrics['suspended_customers']])" />
            <x-dashboard-metric icon="wallet" label="{{ __('Collected this month') }}" :count="$metrics['monthly_revenue']" decimals="2" currency value="{{ number_format($metrics['monthly_revenue'], 2) }}" :sub="__(':value last month', ['value' => '৳'.number_format($metrics['revenue_last_month'], 2)])" :trend="__(':value vs last month', ['value' => ($metrics['revenue_delta'] >= 0 ? '+' : '').$metrics['revenue_delta'].'%'])" :trend-up="$metrics['revenue_delta'] >= 0" />
            <x-dashboard-metric icon="receipt" label="{{ __('Open invoices') }}" :count="$metrics['pending_billing']" value="{{ number_format($metrics['pending_billing']) }}" :sub="__(':amount billed · :count overdue', ['amount' => '৳'.number_format($metrics['outstanding'], 2), 'count' => $metrics['overdue_count']])" />
            <x-dashboard-metric icon="signal" label="{{ __('Online devices') }}" :count="$metrics['online_devices']" value="{{ number_format($metrics['online_devices']) }}" :sub="__(':pct% of :total devices', ['pct' => $metrics['total_devices'] > 0 ? round($metrics['online_devices'] / $metrics['total_devices'] * 100) : 0, 'total' => number_format($metrics['total_devices'])])" />
        @endif
    </section>
                    ...
                    <div class="mb-4 flex items-center justify-between gap-3">
                        ...
                        <div class="flex shrink-0 items-center gap-3">
                            <span class="inline-flex items-center gap-1.5 text-theme-xs font-medium text-gray-500 dark:text-gray-400">
                                <span class="size-2 rounded-full bg-brand-500"></span> {{ __('Collected') }}
                            </span>
                            <span class="inline-flex items-center gap-1.5 text-theme-xs font-medium text-gray-500 dark:text-gray-400">
                                <span class="size-2 rounded-full" style="background:#9CB9FF"></span> {{ __('Billed') }}
                            </span>
                        </div>
                    </div>
            ...
            <div class="col-span-12 md:col-span-6 xl:col-span-4">
                <div class="rounded-2xl border border-gray-200 bg-white p-5 dark:border-gray-800 dark:bg-white/[0.03] sm:p-6">
                    ...
                    @if($chartEmpty($charts['signups']))
                        <p class="grid h-[270px] place-items-center text-center text-theme-sm text-gray-500 dark:text-gray-400">{{ __('No customers added yet.') }}</p>
                    @else
            ...
            <div class="col-span-12 md:col-span-6 xl:col-span-4">
                <div class="rounded-2xl border border-gray-200 bg-white p-5 dark:border-gray-800 dark:bg-white/[0.03] sm:p-6">
                    ...
                    @if($chartEmpty($charts['methods']))
                        <p class="grid h-[270px] place-items-center text-center text-theme-sm text-gray-500 dark:text-gray-400">{{ __('No payments recorded yet.') }}</p>
                    @else
            ...
            <div class="col-span-12 xl:col-span-6">
                <div class="rounded-2xl border border-gray-200 bg-white p-5 dark:border-gray-800 dark:bg-white/[0.03] sm:p-6">
                    <div class="mb-4 flex items-center justify-between gap-3">
                        ...
                        <a href="{{ route('payments') }}" class="text-theme-sm font-medium text-brand-500 hover:text-brand-600 dark:text-brand-400">{{ __('View payments') }}</a>
                    </div>
                    <div class="divide-y divide-gray-100 dark:divide-gray-800">
                        ...
                        @empty
                            <p class="py-10 text-center text-theme-sm text-gray-500 dark:text-gray-400">{{ __('No collections in this workspace yet.') }}</p>
                        @endforelse
                    </div>
                </div>
            </div>
            <div class="col-span-12 xl:col-span-6">
                <div class="rounded-2xl border border-gray-200 bg-white p-5 dark:border-gray-800 dark:bg-white/[0.03] sm:p-6">
                    <div class="mb-4 flex items-center justify-between gap-3">
                        ...
                        <a href="{{ route('billing') }}" class="text-theme-sm font-medium text-brand-500 hover:text-brand-600 dark:text-brand-400">{{ __('Open billing') }}</a>
                    </div>
                    <div class="divide-y divide-gray-100 dark:divide-gray-800">
                        ...
                        @empty
                            <p class="py-10 text-center text-theme-sm text-gray-500 dark:text-gray-400">{{ __('No overdue invoices.') }}</p>
                        @endforelse
                    </div>
                </div>
            </div>
