<div class="space-y-6">
    @php
        ...
        $beeCollectionActive = (($tenant?->settings['collection']['mode'] ?? 'bee') === 'bee');
    @endphp
    ...
    @if($viewMode === 'index')
        ...
        <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div>
                <p class="text-theme-xs font-semibold uppercase tracking-wider text-brand-500 dark:text-brand-400">{{ __('Billing') }}</p>
                <h1 class="mt-1 text-title-sm font-bold text-gray-800 dark:text-white/90">{{ __('Invoices') }}</h1>
                <p class="mt-1 text-theme-sm text-gray-500 dark:text-gray-400">{{ __('Create line-item invoices, run recurring billing and monitor outstanding balances.') }}</p>
            </div>
            <div class="flex flex-wrap items-center gap-3">
                <button
                    type="button"
                    @click="$dispatch('confirm-action', {
                        title: '{{ __('Run recurring billing') }}',
                        message: '{{ __('Generate invoices now for every active subscription that is due? Customers already billed for the current period will be skipped — safe to run anytime.') }}',
                        confirmText: '{{ __('Generate now') }}',
        ...
        <section class="grid grid-cols-1 gap-4 sm:grid-cols-2 md:gap-6 xl:grid-cols-4">
            <div class="rounded-2xl border border-gray-200 bg-white p-5 dark:border-gray-800 dark:bg-white/[0.03]">
                <p class="text-theme-xs font-semibold uppercase tracking-wider text-gray-500 dark:text-gray-400">{{ __('Open invoices') }}</p>
                <p class="mt-2 text-2xl font-bold text-gray-800 dark:text-white/90">{{ $summary['open_count'] }}</p>
                <p class="mt-1 text-theme-xs text-gray-500 dark:text-gray-400">{{ __('Draft, pending & overdue') }}</p>
            </div>
            <div class="rounded-2xl border border-gray-200 bg-white p-5 dark:border-gray-800 dark:bg-white/[0.03]">
                <p class="text-theme-xs font-semibold uppercase tracking-wider text-gray-500 dark:text-gray-400">{{ __('Collected') }}</p>
                <p class="mt-2 truncate text-2xl font-bold text-success-600 dark:text-success-500">৳{{ number_format($summary['collected'], 2) }}</p>
                <p class="mt-1 text-theme-xs text-gray-500 dark:text-gray-400">{{ __('Successful payments') }}</p>
            </div>
            <div class="rounded-2xl border border-gray-200 bg-white p-5 dark:border-gray-800 dark:bg-white/[0.03]">
                <p class="text-theme-xs font-semibold uppercase tracking-wider text-gray-500 dark:text-gray-400">{{ __('Outstanding') }}</p>
                <p class="mt-2 truncate text-2xl font-bold text-warning-600 dark:text-warning-500">৳{{ number_format($summary['outstanding'], 2) }}</p>
                <p class="mt-1 text-theme-xs text-gray-500 dark:text-gray-400">{{ __('Yet to collect') }}</p>
            </div>
            <div class="rounded-2xl border border-gray-200 bg-white p-5 dark:border-gray-800 dark:bg-white/[0.03]">
                <p class="text-theme-xs font-semibold uppercase tracking-wider text-gray-500 dark:text-gray-400">{{ __('Overdue') }}</p>
                <p class="mt-2 truncate text-2xl font-bold text-error-600 dark:text-error-500">৳{{ number_format($summary['overdue'], 2) }}</p>
                <p class="mt-1 text-theme-xs text-gray-500 dark:text-gray-400">{{ __('Past due balance') }}</p>
            </div>
        </section>

        <!-- Invoices table -->
        <x-table heading="{{ __('All invoices') }}" :description="$invoices->total() === 1 ? __('Showing :count invoice', ['count' => number_format($invoices->total())]) : __('Showing :count invoices', ['count' => number_format($invoices->total())])" :paginator="$invoices">
            <x-slot:toolbar>
            ...
            <table class="min-w-full">
                <thead class="border-b border-gray-100 bg-gray-50/50 dark:border-gray-800 dark:bg-white/[0.02]">
                    <tr>
                        ...
                        <th class="px-5 py-3.5 text-left text-theme-xs font-medium uppercase tracking-wide text-gray-500 dark:text-gray-400">{{ __('Due date') }}</th>
                        <th class="px-5 py-3.5 text-right text-theme-xs font-medium uppercase tracking-wide text-gray-500 dark:text-gray-400">{{ __('Total') }}</th>
                    ...
                    @forelse($invoices as $invoice)
                        <tr class="transition-colors hover:bg-gray-50/70 dark:hover:bg-white/[0.02]">
                            ...
                            <td class="px-5 py-4">
                                ...
                                @else
                                    <span class="text-theme-xs text-gray-500 dark:text-gray-400">{{ __('Deleted customer') }}</span>
                                @endif
                            </td>
                            ...
                            <td class="px-5 py-4 text-right">
                                ...
                                @if((float) $invoice->paid_amount > 0)
                                    <div class="mt-0.5 text-theme-xs font-normal text-success-600 dark:text-success-400">{{ __('Paid ৳:amount', ['amount' => number_format($invoice->paid_amount, 2)]) }}</div>
                                @endif
                                @if((float) $invoice->outstanding_amount > 0)
                                    <div class="text-theme-xs font-normal text-warning-600 dark:text-warning-400">{{ __('Due ৳:amount', ['amount' => number_format($invoice->outstanding_amount, 2)]) }}</div>
                                @endif
                            </td>
                            <td class="px-5 py-4">
                                <span class="inline-flex rounded-full px-2.5 py-1 text-theme-xs font-medium capitalize {{ $invoiceBadge($invoice->status) }}">{{ $invoiceStatusNames[$invoice->status] ?? $invoice->status }}</span>
                            </td>
                            <td class="px-5 py-4">
                                <div class="flex items-center justify-end gap-1.5">
                                    <button type="button" wire:click="viewInvoice({{ $invoice->id }})" title="{{ __('Open full page') }}" class="grid h-8 w-8 place-items-center rounded-lg border border-gray-200 text-gray-500 transition hover:border-brand-300 hover:bg-brand-50 hover:text-brand-600 dark:border-gray-800 dark:text-gray-400 dark:hover:border-brand-500/40 dark:hover:bg-brand-500/10 dark:hover:text-brand-400">
                                    ...
                                    @if(in_array($invoice->status, ['pending', 'overdue'], true) && (float) $invoice->outstanding_amount > 0 && $beeCollectionActive)
                                        <button type="button" title="{{ __('Copy bKash payment link') }}" onclick="copyBeePayLink('{{ route('bee-pay.invoice', ['invoice' => $invoice->id]) }}', this)" class="grid h-8 w-8 place-items-center rounded-lg border border-gray-200 text-gray-500 transition hover:border-pink-300 hover:bg-pink-50 hover:text-pink-600 dark:border-gray-800 dark:text-gray-400 dark:hover:border-pink-500/40 dark:hover:bg-pink-500/10 dark:hover:text-pink-400">
                                            <svg class="size-4 stroke-current" viewBox="0 0 24 24" fill="none" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><rect x="9" y="9" width="13" height="13" rx="2"/><path d="M5 15H4a2 2 0 0 1-2-2V4a2 2 0 0 1 2-2h9a2 2 0 0 1 2 2v1"/></svg>
                                        </button>
                                    @endif
                                    <button type="button" wire:click="edit({{ $invoice->id }})" title="{{ __('Edit invoice') }}" class="grid h-8 w-8 place-items-center rounded-lg border border-gray-200 text-gray-500 transition hover:border-brand-300 hover:bg-brand-50 hover:text-brand-600 dark:border-gray-800 dark:text-gray-400 dark:hover:border-brand-500/40 dark:hover:bg-brand-500/10 dark:hover:text-brand-400">
                                    ...
                                    <button
                                        ...
                                        @click="$dispatch('confirm-action', {
                                            title: '{{ __('Delete invoice') }}',
                                            message: '{{ __('Delete invoice :number? This cannot be undone. Payment history is kept but detached from this invoice.', ['number' => $invoice->invoice_number]) }}',
                        ...
                        <tr>
                            <td colspan="6" class="px-5 py-12 text-center">
                                <div class="mx-auto max-w-xs">
                                    <p class="text-theme-sm text-gray-500 dark:text-gray-400">{{ $search || $statusFilter ? __('No invoices match your filters.') : __('No invoices found yet.') }}</p>
                                </div>
                            </td>
                        </tr>
                    @endforelse
    ...
    @elseif($viewMode === 'view')
        ...
        <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div class="flex items-center gap-3">
                <button wire:click="closeView" class="grid h-10 w-10 shrink-0 place-items-center rounded-lg border border-gray-200 text-gray-500 transition hover:bg-gray-100 hover:text-gray-700 dark:border-gray-800 dark:text-gray-400 dark:hover:bg-gray-800 dark:hover:text-white" title="{{ __('Back to invoices') }}">
                ...
                <div>
                    <p class="text-theme-xs font-semibold uppercase tracking-wider text-brand-500 dark:text-brand-400">{{ __('Invoice') }}</p>
            ...
            <div class="flex flex-wrap items-center gap-2">
                ...
                <button
                    ...
                    @click="$dispatch('confirm-action', {
                        title: '{{ __('Delete invoice') }}',
                        message: '{{ __('Delete invoice :number? This cannot be undone. Payment history is kept but detached from this invoice.', ['number' => $inv->invoice_number]) }}',
                        confirmText: '{{ __('Delete') }}',
                        wireMethod: 'delete',
                        wireParams: [{{ $inv->id }}],
                    })"
        ...
        @if((float) $invPaid > 0)
            <div class="flex items-start gap-3 rounded-xl border border-success-200 bg-success-50 px-4 py-3 dark:border-success-500/20 dark:bg-success-500/10">
                ...
                <p class="text-theme-sm text-success-700 dark:text-success-300">{{ __('This invoice has received ৳:amount in payments. Editing or deleting it keeps the payment history — adjust only when you intend to.', ['amount' => number_format($invPaid, 2)]) }}</p>
            </div>
        @endif
        ...
        <div class="overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-theme-xs dark:border-gray-800 dark:bg-gray-900">
            ...
            <div class="flex flex-col gap-6 border-b border-gray-100 px-5 py-6 sm:flex-row sm:items-start sm:justify-between sm:px-8 dark:border-gray-800">
                ...
                <div class="text-left sm:text-right">
                    <p class="text-theme-xs font-medium uppercase tracking-wide text-gray-500 dark:text-gray-400">{{ __('Invoice') }}</p>
                    <p class="text-title-sm mt-0.5 font-bold text-gray-900 dark:text-white">{{ $inv->invoice_number }}</p>
                    <span class="mt-2 inline-flex rounded-full px-2.5 py-1 text-theme-xs font-medium capitalize {{ $invoiceBadge($inv->status) }}">{{ $invoiceStatusNames[$inv->status] ?? $inv->status }}</span>
                </div>
            </div>
            ...
            <div class="grid gap-6 px-5 py-6 sm:grid-cols-3 sm:px-8">
                <div>
                    <p class="text-theme-xs font-semibold uppercase tracking-wide text-gray-500 dark:text-gray-400">{{ __('Billed to') }}</p>
                    <p class="mt-2 text-theme-sm font-semibold text-gray-800 dark:text-white/90">{{ $inv->customer?->name ?? __('Deleted customer') }}</p>
                ...
                <div class="space-y-2">
                    <div class="flex justify-between gap-4 text-theme-sm">
                        <span class="text-gray-500 dark:text-gray-400">{{ __('Issued') }}</span>
                    ...
                    @if($inv->billing_period_start)
                        <div class="flex justify-between gap-4 text-theme-sm">
                            <span class="text-gray-500 dark:text-gray-400">{{ __('Period') }}</span>
                ...
                <div class="space-y-2 sm:ml-auto sm:w-52">
                    ...
                    <div class="flex justify-between gap-4 border-t border-gray-100 pt-2 text-theme-sm dark:border-gray-800">
                        <span class="font-medium text-gray-700 dark:text-gray-300">{{ __('Balance due') }}</span>
                ...
                <table class="w-full min-w-[540px]">
                    <thead>
                        <tr class="border-y border-gray-200 bg-gray-50/70 text-left dark:border-gray-800 dark:bg-white/[0.02]">
                            <th class="py-3 pr-4 text-theme-xs font-semibold uppercase tracking-wide text-gray-500 dark:text-gray-400">{{ __('Description') }}</th>
                            <th class="w-16 py-3 pr-4 text-right text-theme-xs font-semibold uppercase tracking-wide text-gray-500 dark:text-gray-400">{{ __('Qty') }}</th>
                            <th class="w-28 py-3 pr-4 text-right text-theme-xs font-semibold uppercase tracking-wide text-gray-500 dark:text-gray-400">{{ __('Rate') }}</th>
                            <th class="w-28 py-3 text-right text-theme-xs font-semibold uppercase tracking-wide text-gray-500 dark:text-gray-400">{{ __('Amount') }}</th>
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-gray-100 dark:divide-gray-800">
                        ...
                        @empty
                            <tr><td colspan="4" class="py-6 text-center text-theme-sm text-gray-500 dark:text-gray-400">{{ __('No line items on this invoice.') }}</td></tr>
                        @endforelse
                    </tbody>
                </table>

                <div class="mt-4 ml-auto w-full max-w-xs space-y-2">
                    <div class="flex justify-between text-theme-sm text-gray-600 dark:text-gray-400"><span>{{ __('Subtotal') }}</span><span>৳{{ number_format($inv->subtotal, 2) }}</span></div>
                    <div class="flex justify-between text-theme-sm text-gray-600 dark:text-gray-400"><span>{{ __('Tax') }}</span><span>৳{{ number_format($inv->tax_amount, 2) }}</span></div>
                    <div class="flex justify-between border-t border-gray-200 pt-2 text-theme-sm font-semibold text-gray-800 dark:border-gray-800 dark:text-white/90"><span>{{ __('Total') }}</span><span>৳{{ number_format($inv->total, 2) }}</span></div>
                </div>
            </div>
            ...
            @if($inv->payments->isNotEmpty())
                <div class="border-t border-gray-100 px-5 py-5 sm:px-8 dark:border-gray-800">
                    <p class="text-theme-xs font-semibold uppercase tracking-wide text-gray-500 dark:text-gray-400">{{ __('Payment history') }}</p>
            ...
            @if($beeCollectionActive && (float) $invOutstanding > 0)
                @php $payUrl = route('bee-pay.invoice', ['invoice' => $inv->id]); @endphp
                <div class="px-5 pt-5 sm:px-8">
                    <div class="rounded-xl border border-pink-200 bg-pink-50/60 px-4 py-3.5 dark:border-pink-500/25 dark:bg-pink-500/10">
                        <div class="flex items-center justify-between gap-3">
                            <div class="min-w-0">
                                ...
                                <p class="mt-1 text-theme-xs leading-5 text-gray-600 dark:text-gray-400">{{ __('Send this link to the customer — they pay securely through bKash and the payment is recorded automatically.') }}</p>
                            </div>
                            <div class="flex shrink-0 items-center gap-1.5">
                                <a href="{{ $payUrl }}" target="_blank" title="{{ __('Open payment page') }}" class="grid h-9 w-9 place-items-center rounded-lg border border-gray-200 bg-white text-gray-500 transition hover:border-brand-300 hover:bg-brand-50 hover:text-brand-600 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-400 dark:hover:border-brand-500/40 dark:hover:text-brand-400">
                                    <svg class="size-4 stroke-current" viewBox="0 0 24 24" fill="none" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M18 13v6a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V8a2 2 0 0 1 2-2h6"/><polyline points="15 3 21 3 21 9"/><line x1="10" y1="14" x2="21" y2="3"/></svg>
                                </a>
                                <button type="button" onclick="copyBeePayLink('{{ $payUrl }}', this)" title="{{ __('Copy payment link') }}" class="grid h-9 w-9 place-items-center rounded-lg border border-gray-200 bg-white text-gray-500 transition hover:border-pink-300 hover:bg-pink-50 hover:text-pink-600 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-400 dark:hover:border-pink-500/40 dark:hover:bg-pink-500/10 dark:hover:text-pink-400">
                                    <svg class="size-4 stroke-current" viewBox="0 0 24 24" fill="none" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><rect x="9" y="9" width="13" height="13" rx="2"/><path d="M5 15H4a2 2 0 0 1-2-2V4a2 2 0 0 1 2-2h9a2 2 0 0 1 2 2v1"/></svg>
                                </button>
                            </div>
                        </div>
            ...
            <div class="px-5 pb-5 sm:px-8">
                <x-payment-instructions :tenant="$tenant" title="{{ __('Payment instructions') }}" />
            </div>
            ...
            <div class="border-t border-gray-100 bg-gray-50/50 px-5 py-5 sm:flex sm:items-center sm:justify-between sm:px-8 dark:border-gray-800 dark:bg-white/[0.02]">
                <div class="min-w-0">
                    <p class="text-theme-xs text-gray-500 dark:text-gray-400">{{ __('Thank you for your business.') }}</p>
                    <p class="mt-0.5 text-theme-xs text-gray-500 dark:text-gray-400">{{ __('Questions about this invoice? Contact :brand.', ['brand' => $brandFooter]) }}</p>
                </div>
                <p class="mt-3 shrink-0 text-theme-xs font-medium text-gray-400 sm:mt-0 dark:text-gray-500">{{ __(':number · generated by :brand', ['number' => $inv->invoice_number, 'brand' => $brandFooter]) }}</p>
            </div>
        </div>
    ...
    @else
        ...
        <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div>
                <p class="text-theme-xs font-semibold uppercase tracking-wider text-brand-500 dark:text-brand-400">{{ __('Billing') }}</p>
                <h1 class="mt-1 text-title-sm font-bold text-gray-800 dark:text-white/90">{{ $isEditing ? __('Edit invoice') : __('Generate invoice') }}</h1>
                <p class="mt-1 text-theme-sm text-gray-500 dark:text-gray-400">{{ $isEditing ? __('Adjust the line items, tax and due date.') : __('Create a line-item invoice for a customer.') }}</p>
            </div>
            <div class="flex flex-wrap items-center gap-3">
        ...
        <form wire:submit="save" class="space-y-6">
            ...
            <section class="rounded-2xl border border-gray-200 bg-white p-5 shadow-theme-xs dark:border-gray-800 dark:bg-white/[0.03] sm:p-6">
                <div class="mb-4">
                    <h2 class="text-base font-semibold text-gray-800 dark:text-white/90">{{ __('Customer & details') }}</h2>
                    <p class="mt-0.5 text-theme-xs text-gray-500 dark:text-gray-400">{{ __('Choose who is billed and when it becomes due.') }}</p>
                </div>
                <div class="grid gap-4 sm:grid-cols-2">
                    <div class="sm:col-span-2">
                        <label for="invoice-customer" class="mb-1.5 block text-theme-sm font-medium text-gray-700 dark:text-gray-400">{{ __('Customer') }}<span class="ml-0.5 text-error-500">*</span></label>
                        <x-search-select wireKey="customer_id" :options="$customerOptions" :value="$customer_id" placeholder="{{ __('Select a customer...') }}" />
                        @error('customer_id') <p class="mt-1.5 text-theme-xs text-error-600 dark:text-error-400">{{ $message }}</p> @enderror
                    </div>
                    <div>
                        <label for="invoice-status" class="mb-1.5 block text-theme-sm font-medium text-gray-700 dark:text-gray-400">{{ __('Status') }}<span class="ml-0.5 text-error-500">*</span></label>
                        <x-search-select wireKey="status" :options="['draft' => __('Draft'), 'pending' => __('Pending'), 'overdue' => __('Overdue'), 'paid' => __('Paid'), 'cancelled' => __('Cancelled')]" :value="$status" placeholder="{{ __('Select status') }}" :searchable="false" />
                        @error('status') <p class="mt-1.5 text-theme-xs text-error-600 dark:text-error-400">{{ $message }}</p> @enderror
                    </div>
                    <div>
                        <label for="invoice-due-date" class="mb-1.5 block text-theme-sm font-medium text-gray-700 dark:text-gray-400">{{ __('Due date') }}<span class="ml-0.5 text-error-500">*</span></label>
            ...
            <section class="rounded-2xl border border-gray-200 bg-white p-5 shadow-theme-xs dark:border-gray-800 dark:bg-white/[0.03] sm:p-6">
                <div class="mb-4 flex items-center justify-between gap-3">
                    <div>
                        <h2 class="text-base font-semibold text-gray-800 dark:text-white/90">{{ __('Invoice items') }}</h2>
                        <p class="mt-0.5 text-theme-xs text-gray-500 dark:text-gray-400">{{ __('Add at least one line item with quantity and unit price.') }}</p>
                    </div>
                ...
                <div class="space-y-3">
                    @foreach($items as $index => $item)
                        <div wire:key="invoice-item-{{ $index }}" class="grid gap-2.5 rounded-xl border border-gray-100 bg-gray-50/60 p-3 sm:grid-cols-[1fr_80px_110px_110px_36px] dark:border-gray-800 dark:bg-white/[0.02]">
                            <div>
                                <input type="text" wire:model="items.{{ $index }}.description" placeholder="{{ __('Description') }}" class="h-10 w-full rounded-lg border border-gray-300 bg-transparent px-3.5 text-theme-sm text-gray-800 shadow-theme-xs placeholder:text-gray-400 focus:border-brand-300 focus:ring-3 focus:ring-brand-500/10 focus:outline-hidden dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30">
                                @error("items.$index.description") <p class="mt-1 text-theme-xs text-error-600 dark:text-error-400">{{ $message }}</p> @enderror
                            </div>
                            <div>
                                <input type="number" min="0.01" step="0.01" wire:model.live="items.{{ $index }}.quantity" placeholder="{{ __('Qty') }}" class="h-10 w-full rounded-lg border border-gray-300 bg-transparent px-3 text-theme-sm text-gray-800 shadow-theme-xs placeholder:text-gray-400 focus:border-brand-300 focus:ring-3 focus:ring-brand-500/10 focus:outline-hidden dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30">
                                @error("items.$index.quantity") <p class="mt-1 text-theme-xs text-error-600 dark:text-error-400">{{ $message }}</p> @enderror
                            </div>
                            <div>
                                <input type="number" min="0" step="0.01" wire:model.live="items.{{ $index }}.unit_price" placeholder="{{ __('Unit price') }}" class="h-10 w-full rounded-lg border border-gray-300 bg-transparent px-3 text-theme-sm text-gray-800 shadow-theme-xs placeholder:text-gray-400 focus:border-brand-300 focus:ring-3 focus:ring-brand-500/10 focus:outline-hidden dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30">
                            ...
                            <button type="button" wire:click="removeItem({{ $index }})" @disabled(count($items) <= 1) title="{{ __('Remove item') }}" class="grid h-10 w-10 place-items-center self-center rounded-lg text-gray-400 transition hover:bg-error-50 hover:text-error-600 disabled:cursor-not-allowed disabled:opacity-40 dark:hover:bg-error-500/10 dark:hover:text-error-400">
                                <svg class="size-4 stroke-current" viewBox="0 0 24 24" fill="none" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><polyline points="3 6 5 6 21 6"/><path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"/><line x1="10" y1="11" x2="10" y2="17"/><line x1="14" y1="11" x2="14" y2="17"/></svg>
                            </button>
                        </div>
                    @endforeach
            ...
            <section class="rounded-2xl border border-gray-200 bg-white p-5 shadow-theme-xs dark:border-gray-800 dark:bg-white/[0.03] sm:p-6">
                <div class="mb-4">
                    <h2 class="text-base font-semibold text-gray-800 dark:text-white/90">{{ __('Totals') }}</h2>
                    <p class="mt-0.5 text-theme-xs text-gray-500 dark:text-gray-400">{{ __('Subtotal is auto-calculated from line items. Total includes tax.') }}</p>
                </div>
                <div class="grid gap-4 sm:grid-cols-3">
                    <div>
                        <label class="mb-1.5 block text-theme-xs font-medium text-gray-500 dark:text-gray-400">{{ __('Subtotal') }}</label>
                    ...
                    <div>
                        <label for="invoice-tax" class="mb-1.5 block text-theme-sm font-medium text-gray-700 dark:text-gray-400">{{ __('Tax amount (BDT)') }}</label>
                    ...
                    <div>
                        <label class="mb-1.5 block text-theme-xs font-medium text-gray-500 dark:text-gray-400">{{ __('Total') }}</label>
            ...
            <div class="sticky bottom-4 flex flex-col-reverse gap-3 rounded-2xl border border-gray-200 bg-white/95 px-5 py-4 shadow-theme-lg backdrop-blur sm:flex-row sm:items-center sm:justify-between dark:border-gray-800 dark:bg-gray-900/95">
                ...
                <div class="flex flex-col-reverse gap-3 sm:flex-row">
                    ...
                    <button type="submit" wire:loading.attr="disabled" class="inline-flex items-center justify-center gap-2 rounded-lg bg-brand-500 px-4 py-2.5 text-theme-sm font-medium text-white shadow-theme-xs transition hover:bg-brand-600 disabled:cursor-not-allowed disabled:opacity-50">
                        <span wire:loading.remove wire:target="save">{{ $isEditing ? __('Update invoice') : __('Generate invoice') }}</span>
